...
class FaceAging_AAE(object):
    ...
    def discriminator_img(self, image, y, gender, is_training=True, reuse_variables=False, enable_tile_label=True, num_hidden_layer_channels=(16, 32, 64, 128), enable_bn=True):
        if reuse_variables:
            tf.get_variable_scope().reuse_variables()
        ...
        if self.version:
            ...
            name = 'D_img_conv' + str(num_layers)
            current = conv2d(
                    input_map=current,
    ...
    def load_checkpoint(self, model_path=None):
        ...
        if checkpoints and checkpoints.model_checkpoint_path:
            ...
            try:
                ...
                print("\n\tLoading model {}".format(checkpoints_name))
                return True
            except:
                return False
        else:
