...
def conv2d(input_map, num_output_channels, size_kernel=5, stride=2, padding='SAME', name='conv2d'):
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(input_map, kernel, strides=[1, stride, stride, 1], padding=padding)
...
def deconv2d(input_map, output_shape, size_kernel=5, stride=2, padding='SAME', name='deconv2d'):
    with tf.variable_scope(name):
        ...
        deconv = tf.nn.conv2d_transpose(input_map, kernel, strides=[1, stride, stride, 1], output_shape=output_shape, padding=padding)
